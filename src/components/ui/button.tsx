...
const buttonVariants = cva(
  "inline-flex items-center gap-2.5 font-mono uppercase rounded-full transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
  {
    variants: {
      variant: {
        primary:
          "bg-ink text-bg border border-ink hover:bg-moss hover:border-moss",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    // Slottable lets us place the consumer's child (e.g. <Link>) alongside the
    // arrow icon when asChild is true. Without it, Slot would reject the extra
    // child and silently drop one of them.
    return (
      <Comp
        ref={ref}
